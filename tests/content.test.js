...
test("B站 Digest 使用统一海蓝色，Note 保持白字深色浮层", async () => {
  const dom = createDom();
  const toolbar = dom.register(".video-toolbar-left");
  const player = dom.register("#bilibili-player");
  ...
  assert.match(digest.style.cssText, /#168cff/i);
  assert.equal(note.style.color, "#fff");
  assert.equal(note.style.background, "rgba(0,0,0,.55)");
});
