...
const EN_TEXT = Object.freeze({
  ...
  "飞书多维表格同步": "Lark Bitable sync",
  "将笔记逐条发送到你配置的飞书多维表格 Webhook。视频笔记、手记和 AI 记使用完全一致的 JSON 字段，便于一个工作流统一新增或更新 record。": "Send notes one at a time to your configured Lark Bitable Webhook. Video notes, memos, and AI notes use the same JSON fields, so one workflow can create or update records consistently.",
  "配置方法": "Setup",
  "在目标多维表格创建自动化流程，触发条件选择“接收到 webhook 时”。": "Create an automation in the target Bitable and choose “When a webhook is received” as the trigger.",
  "复制该流程的 Webhook 地址；在后续操作中添加“新增记录”，映射请求里的": "Copy the workflow Webhook URL. Add “Create record” as the next action and map the request's",
  "字段。": "field.",
  "把地址粘贴到下方并保存。需要凭证校验时，填写自动化流程生成的 Bearer Token。": "Paste the URL below and save it. If credential verification is enabled, enter the Bearer Token generated by the automation.",
  "飞书官方 Webhook 使用说明 ↗": "Lark official Webhook guide ↗",
  "飞书 Webhook 地址": "Lark Webhook URL",
  "Bearer Token（可选）": "Bearer Token (optional)",
  "仅在飞书自动化开启“凭证校验”时填写；失焦后仅显示前 2 位与后 3 位。": "Enter this only when Lark automation credential verification is enabled. When unfocused, only the first 2 and last 3 characters are shown.",
  "保存笔记后自动同步到飞书": "Automatically sync after saving a note",
  "保存飞书 Webhook": "Save Lark Webhook",
  "测试连接": "Test connection",
  "测试连接会发送一条": "Testing sends one",
  "记录；批量同步按飞书 Webhook 限流逐条发送。地址、Token 仅保存在本机扩展存储。": "record. Bulk syncing sends one request at a time to respect Lark Webhook rate limits. The URL and token are stored only in local extension storage.",
  "飞书 Webhook 已保存并授权": "Lark Webhook saved and authorized",
  "正在测试飞书 Webhook…": "Testing Lark Webhook…",
  "飞书 Webhook 连接正常（已发送测试记录）": "Lark Webhook is reachable (a test record was sent)",
  "没有获得飞书 Webhook 的域名权限。": "Host permission was not granted for the Lark Webhook.",
});
...
const larkWebhookUrl = document.getElementById("larkWebhookUrl");
const larkWebhookBearerToken = document.getElementById("larkWebhookBearerToken");
const larkWebhookAutoSync = document.getElementById("larkWebhookAutoSync");
const larkWebhookStatus = document.getElementById("larkWebhookStatus");
...
let larkWebhook = BILI_SETTINGS.normalizeLarkWebhook();
...
function currentAppSettings() {
  return BILI_SETTINGS.normalizeAppSettings({
    ...
    overviewPrompts,
    larkWebhook: {
      url: larkWebhookUrl.value,
      bearerToken: apiKeyValue(larkWebhookBearerToken),
      autoSync: larkWebhookAutoSync.checked,
    },
  });
}

function larkWebhookFromForm() {
  return BILI_SETTINGS.normalizeLarkWebhook({
    url: larkWebhookUrl.value,
    bearerToken: apiKeyValue(larkWebhookBearerToken),
    autoSync: larkWebhookAutoSync.checked,
  });
...
async function saveLarkWebhook() {
  const webhook = larkWebhookFromForm();
  const check = BILI_SETTINGS.validateLarkWebhook(webhook);
  if (!check.ok) {
    showStatus(larkWebhookStatus, check.error, { sticky: true, error: true });
    return false;
  }
  try {
    await requestMissingOrigins([check.origin], "没有获得飞书 Webhook 的域名权限。");
    const stored = await chromeApi.storage.local.get([storageKey, BILI_SETTINGS.LEGACY_STORAGE_KEY]);
    const settings = BILI_SETTINGS.normalizeAppSettings(
      stored[storageKey] ?? stored[BILI_SETTINGS.LEGACY_STORAGE_KEY],
    );
    const next = BILI_SETTINGS.normalizeAppSettings({ ...settings, larkWebhook: webhook });
    await chromeApi.storage.local.set({ [storageKey]: next });
    larkWebhook = next.larkWebhook;
    larkWebhookUrl.value = larkWebhook.url;
    setApiKeyValue(larkWebhookBearerToken, larkWebhook.bearerToken);
    larkWebhookAutoSync.checked = larkWebhook.autoSync;
    showStatus(larkWebhookStatus, "飞书 Webhook 已保存并授权");
    return true;
  } catch (error) {
    showStatus(larkWebhookStatus, `保存失败：${error.message}`, { sticky: true, error: true });
    return false;
  }
}

async function testLarkWebhook() {
  const button = document.getElementById("testLarkWebhookBtn");
  button.disabled = true;
  try {
    if (!(await saveLarkWebhook())) return;
    showStatus(larkWebhookStatus, "正在测试飞书 Webhook…", { sticky: true });
    const result = await chromeApi.runtime.sendMessage({ action: "testLarkWebhook" });
    if (!result?.success) throw new Error(result?.message || "测试失败，请重试。");
    showStatus(larkWebhookStatus, "飞书 Webhook 连接正常（已发送测试记录）");
  } catch (error) {
    showStatus(larkWebhookStatus, `测试失败：${error.message}`, { sticky: true, error: true });
  } finally {
  ...
  const origins = [
    ...new Set(
      [
        ...
        BILI_SETTINGS.validateLarkWebhook(appSettings.larkWebhook).origin,
      ].filter(Boolean),
    ),
  ];
...
async function load() {
  ...
  noteLimit.value = savedNoteLimit;
  larkWebhook = settings.larkWebhook;
  larkWebhookUrl.value = larkWebhook.url;
  enableApiKeyMask(larkWebhookBearerToken);
  setApiKeyValue(larkWebhookBearerToken, larkWebhook.bearerToken);
  larkWebhookAutoSync.checked = larkWebhook.autoSync;
...
document.getElementById("saveLarkWebhookBtn").addEventListener("click", saveLarkWebhook);
document.getElementById("testLarkWebhookBtn").addEventListener("click", testLarkWebhook);
for (const button of document.querySelectorAll("[data-language]")) {
