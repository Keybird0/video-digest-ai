...
const EN_TEXT = Object.freeze({
  ...
  "将笔记逐条发送到你配置的飞书多维表格 Webhook。视频笔记、手记和 AI 记使用完全一致的 JSON 字段，便于一个工作流统一新增或更新 record。": "Send notes one at a time to your configured Lark Bitable Webhook. Video notes, memos, and AI notes all use the same JSON fields, so one workflow can create or update records consistently.",
  "配置方法": "Setup",
...
async function saveLarkWebhook() {
  ...
  try {
    ...
    showStatus(larkWebhookStatus, `保存失败：${error.message}`, {
      sticky: true,
      error: true,
    });
    return false;
...
async function testLarkWebhook() {
  ...
  try {
    // 先保存，避免测试时 background 读到旧地址或旧 Token。
    ...
    showStatus(larkWebhookStatus, `测试失败：${error.message}`, {
      sticky: true,
      error: true,
    });
  } finally {
