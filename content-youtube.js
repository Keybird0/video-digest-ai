...
function createDigestButton() {
  ...
  // Style the button — rounded pill in the shared sea-blue accent, sized to sit
  ...
  digestButton.style.cssText = `
    ...
    box-shadow: 0 2px 8px rgba(0, 86, 190, 0.3);
    flex: 0 0 auto;
    align-self: center;
    width: max-content;
  ...
  digestButton.addEventListener("mouseenter", () => {
    digestButton.style.background = DIGEST_BLUE_HOVER;
    digestButton.style.transform = "scale(1.02)";
  });

  digestButton.addEventListener("mouseleave", () => {
    digestButton.style.background = DIGEST_BLUE;
...
function injectNoteButton() {
  ...
  // Follow Bilibili's high-contrast player treatment: white text on a neutral dark veil.
  ...
  noteButton.addEventListener("mouseenter", () => {
    noteButton.style.background = NOTE_BACKGROUND_HOVER;
  ...
  noteButton.addEventListener("mouseleave", () => {
    noteButton.style.background = NOTE_BACKGROUND;
...
function showNoteSavedToast(note) {
  ...
  toast.innerHTML = `
    <div style="font-weight: 700; margin-bottom: 6px; color: ${DIGEST_BLUE};">📝 Note saved</div>
    ...
    <div style="margin-top: 10px; font-size: 11px;">
      <a href="${escapeHtmlForContent(note.timestampedUrl)}" style="color: ${DIGEST_BLUE}; font-weight: 600; text-decoration: none;">🔗 Copy link</a>
    </div>
  `;
