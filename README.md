...
- 可选接入飞书多维表格 Webhook：视频笔记、手记和 AI 记均使用统一 JSON 字段，可逐条同步、一次同步全部笔记，或在保存后自动同步。
...
在设置页“飞书多维表格同步”中粘贴自动化流程的 Webhook 地址；如果流程启用了凭证校验，再填写对应的 Bearer Token。创建工作流时选择“接收到 Webhook 时”触发器，并在后续“新增记录”步骤中映射请求体的 `note` 字段。具体流程请参考[飞书官方 Webhook 使用说明](https://www.feishu.cn/hc/zh-CN/articles/612376356355-%E4%BD%BF%E7%94%A8%E8%87%AA%E5%8A%A8%E5%8C%96%E6%B5%81%E7%A8%8B%E7%9A%84-webhook-%E8%A7%A6%E5%8F%91)。

每次请求都会发送相同结构的一条笔记记录，便于同一个工作流处理所有笔记类型；可用 `note.id` 作为唯一键实现去重或更新。侧边栏的笔记卡片提供“同步飞书”，工具栏提供“同步全部笔记”；批量同步会逐条发送并遵守保守限流。Webhook 地址和 Token 仅存于本机扩展存储，笔记只会在你手动同步或开启自动同步后发送。
